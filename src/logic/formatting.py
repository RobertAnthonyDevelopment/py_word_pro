...
class FormatManager:
    def __init__(self, editor, root):
        ...
        self._typing_pending_anchor = None
    ...
    def _on_keypress_capture_insert(self, evt=None):
        ...
        # Skip non-text/control keypresses to keep typing path lightweight.
        try:
            if evt is not None:
                if evt.state & 0x4:  # Control/Cmd shortcuts
                    return
                if evt.keysym in {
                    'Shift_L', 'Shift_R', 'Control_L', 'Control_R', 'Alt_L', 'Alt_R',
                    'Meta_L', 'Meta_R', 'Caps_Lock', 'Escape', 'Left', 'Right',
                    'Up', 'Down', 'Prior', 'Next', 'Home', 'End', 'Insert',
                }:
                    return
                if not evt.char and evt.keysym not in {'Return', 'Tab', 'space'}:
                    return
        except Exception:
            pass

        # Coalesce many rapid keypresses into one post-idle style application.
        if self._typing_pending_anchor is not None:
            return
        try:
            self._typing_pending_anchor = self.editor.index('insert')
        except tk.TclError:
            self._typing_pending_anchor = None
            return
        ...
        try:
            self.editor.after_idle(self._flush_pending_typed_range)
        except Exception:
            self._typing_pending_anchor = None
    ...
    def _flush_pending_typed_range(self):
        before = self._typing_pending_anchor
        self._typing_pending_anchor = None
        if before:
            self._apply_typing_to_newly_inserted(before)
    ...
    def apply_font_family(self, name):
        self._note_format_activity()
        self._checkpoint()
        self.body_font = str(name or self.body_font)
        try:
            if self.editor.tag_ranges('sel'):
                start, end = self.editor.index('sel.first'), self.editor.index('sel.last')
                before = self._snapshot_style_ranges(start, end)
    ...
    def _clamp_font_size(self, value, minimum=6, maximum=288):
        try:
            numeric = int(value)
        except Exception:
            numeric = int(self.default_size)
        return max(int(minimum), min(int(maximum), numeric))

    def adjust_font_size(self, delta):
        """Increase or decrease font size on selection or typing context."""
        self._note_format_activity()
        self._checkpoint()
        ...
        try:
            if self.editor.tag_ranges("sel"):
                ...
                before = self._snapshot_style_ranges(start, end)
                segments = list(self._iter_style_segments(start, end))
                if not segments and self.editor.compare(start, "<", end):
                    segments = [(start, end, self._get_effective_spec_at(start))]
                self._remove_style_tags_in_range(start, end)
                for s, e, spec in segments:
                    spec = dict(spec)
                    new_size = self._clamp_font_size(int(spec.get("size", self.default_size)) + delta)
                    spec["size"] = new_size
                    tname = self._ensure_style_tag(
                        family=spec["family"],
                        size=int(spec["size"]),
        ...
        new_size = self._clamp_font_size(int(base.get("size", self.default_size)) + delta)
        base["size"] = new_size
        self.default_size = new_size
        self._typing_spec = base
        # Keep typing mode active for no-selection size changes so next typed
        # text always reflects the new size immediately.
        self._typing_enabled = True
        self._checkpoint()
        return True

    def _resolve_text_style_profile(self, style_name: str):
        key = str(style_name or "").strip().lower()
        base_body_font = str(self.body_font or self.default_font or "Calibri")
        profiles = {
            "body": {"family": base_body_font, "size": 11, "b": False, "i": False, "u": False, "o": False},
            "title": {"family": base_body_font, "size": 32, "b": True, "i": False, "u": False, "o": False},
            "subtitle": {"family": base_body_font, "size": 20, "b": False, "i": True, "u": False, "o": False},
            "heading 1": {"family": base_body_font, "size": 24, "b": True, "i": False, "u": False, "o": False},
            "heading 2": {"family": base_body_font, "size": 18, "b": True, "i": False, "u": False, "o": False},
            "heading 3": {"family": base_body_font, "size": 14, "b": True, "i": False, "u": False, "o": False},
            "quote": {"family": base_body_font, "size": 13, "b": False, "i": True, "u": False, "o": False},
            "caption": {"family": base_body_font, "size": 10, "b": False, "i": True, "u": False, "o": False},
            "code": {"family": "Consolas", "size": 11, "b": False, "i": False, "u": False, "o": False},
        }
        profile = dict(profiles.get(key, profiles["body"]))
        profile["size"] = self._clamp_font_size(profile.get("size", self.default_size))
        return profile

    def apply_text_style(self, style_name: str):
        """Apply a named text style to selection or current line."""
        self._note_format_activity()
        self._checkpoint()

        profile = self._resolve_text_style_profile(style_name)
        applied_range = False
        ...
        try:
            if self.editor.tag_ranges("sel"):
                ...
                start, end = self.editor.index("insert linestart"), self.editor.index("insert lineend")
            ...
            if self.editor.compare(start, "<", end):
                ...
                if not segments:
                    segments = [(start, end, self._get_effective_spec_at(start))]
                self._remove_style_tags_in_range(start, end)
                for s, e, _spec in segments:
                    tname = self._ensure_style_tag(
                        family=profile["family"],
                        size=int(profile["size"]),
                        b=bool(profile["b"]),
                        italic=bool(profile["i"]),
                        underline=bool(profile["u"]),
                        overstrike=bool(profile["o"]),
                    )
                    self.editor.tag_add(tname, s, e)
                after = self._snapshot_style_ranges(start, end)
        ...
        self.default_font = str(profile["family"])
        self.default_size = int(profile["size"])
        if str(style_name or "").strip().lower() != "code":
        ...
        # Keep typing mode active for style presets, including "Body", so
        # future typed text reliably uses the selected preset.
        self._typing_enabled = True
        self._checkpoint()
        return applied_range or True
