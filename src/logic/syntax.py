...
class SyntaxHighlighter:
    ...
    def trigger(self):
        if self.timer:
            self.editor.after_cancel(self.timer)
    ...
    def highlight(self):
        ...
        for match in re.finditer(r"```(.*?)```", content, re.DOTALL):
            ...
            for pattern, tag in self.rules:
                for pattern_match in re.finditer(pattern, block_content):
                    start_idx = f"1.0 + {start_offset + pattern_match.start()} chars"
                    end_idx = f"1.0 + {start_offset + pattern_match.end()} chars"
